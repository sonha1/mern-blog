...
export const getGroups = (token) => async (dispatch) => {
  try {
    ...
    const res = await getDataAPI("group", token);
...
export const deleteGroup =
  ...
  async (dispatch) => {
    ...
    try {
      const res = await deleteDataAPI(`group/${group._id}`, auth.token);
...
export const leaveGroup =
  ...
  async (dispatch) => {
    const newGroup = {
      ...group,
      users: group.users.filter((user) => group.users.includes(auth.user._id)),
    };
...
export const incrementDelete =
  ({ groupId, auth }) =>
  async (dispatch) => {
    try {
      await patchDataAPI(`group/incrementDelete/${groupId}`);
    } catch (err) {
      dispatch({
        type: GLOBALTYPES.ALERT,
        payload: { error: err.response.data.msg },
...
export const addRequest =
  ({ auth, groupId, group, userId, socket }) =>
  async (dispatch) => {
    const newGroup = { ...group, users: [...group.users, auth.user] };
...
export const denyRequest =
  ({ group, groupId, auth, user, socket }) =>
  async (dispatch) => {
    const newGroup = { ...group, users: [...group.users, user._id] };
    dispatch({ type: GROUP_TYPES.UPDATE_GROUP, payload: newGroup });
...
export const inviteUser =
  ({ auth, groupId, group, userId, user, socket }) =>
  async (dispatch) => {
    const newGroup = { ...group, users: [...group.users, auth.user] };
