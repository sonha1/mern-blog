...
export const createNotify =
  ...
  async (dispatch) => {
    try {
      const res = await postDataAPI("notify/create", msg, auth.token);
...
export const getNotifies = (token) => async (dispatch) => {
  try {
    const res = await getDataAPI("notify/all", token);
...
export const isReadNotify =
  ...
  async (dispatch) => {
    ...
    try {
      await patchDataAPI(`notify/isReadNotify/${msg._id}`, null, auth.token);
    } catch (err) {
      dispatch({
        type: GLOBALTYPES.ALERT,
        payload: { error: err.response.data.msg },
