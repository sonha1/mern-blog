...
export const createPost =
  ({ content, images, groupId, grp, auth, socket }) =>
  async (dispatch) => {
    let media = [];
    try {
      ...
      const res = await postDataAPI(
        "post/create",
        { content, images: media, groupId, isHidden: false },
        auth.token
      );
...
export const updatePost =
  ({ content, images, groupId, auth, status }) =>
  async (dispatch) => {
    let media = [];
...
export const unflagPost =
  ({ post, auth }) =>
  async (dispatch) => {
    try {
      const res = await patchDataAPI(
        `post/unflagPost/${post._id}`,
        {
          isFlagged: false,
        },
        auth.token
